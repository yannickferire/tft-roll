...
const OpponentsCopies: React.FC<IOpponentsCopies> = ({ champion, opponentsCopies, setOpponentsCopies }) => {
  return (
    <div className="inline-block w-full bg-crema rounded">
      <h4 className="text-sm mb-1">Opponents's <strong className="font-medium">{champion.name}</strong></h4>
      <div className="flex text-lg w-full justify-between mb-2">
        ...
        <span className="p-2 w-10 inline-block text-center">{opponentsCopies}</span>
        <button
          onClick={() => setOpponentsCopies(opponentsCopies + 1)} 
