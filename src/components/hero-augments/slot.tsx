...
const Slot: React.FC<ISlot> = ({ index, stageSelected, slotsCost, setSlotsCost }) => {
  ...
  return (
    <div className="select flex-1 bg-earlynight text-midday rounded leading-10 hover-effect cursor-pointer transition-all duration-300 ease-in-out">
      <select defaultValue="0" value={slotsCost[index-1]} onChange={handleCostChange} className={`w-full text-crema ${slotsCost[index-1] == 0 ? 'bg-earlynight' : 'bg-'+slotsCost[index-1]+'cost'} pl-4 pr-9 z-10 relative rounded focus:outline-none focus:ring focus:ring-midday`}>
